<?php
session_start();
if (isset($_GET['lang']) && in_array($_GET['lang'], ['th', 'en'])) {
    $_SESSION['lang'] = $_GET['lang'];
}
$lang = isset($_SESSION['lang']) ? $_SESSION['lang'] :  'th';

$t = [
    'th' => [
        'nav_home' => 'หน้าแรก', 'nav_rooms' => 'ห้องพัก', 'nav_book' => 'จองห้องพัก',
        'title' => 'ค้นหาใบเสร็จรับเงิน',
        'search_btn' => 'ค้นหา',
        'placeholder' => 'กรอกรหัสการจอง เช่น YC-123456',
        'not_found' => 'ไม่พบข้อมูลการจองรหัสนี้ กรุณาตรวจสอบอีกครั้ง',
        'status_pending' => 'PENDING',
        'status_confirmed' => 'PAID',
        'status_cancelled' => 'CANCELLED',
        'download_png' => 'ดาวน์โหลด PNG',
    ],
    'en' => [
        'nav_home' => 'Home', 'nav_rooms' => 'Rooms', 'nav_book' => 'Booking',
        'title' => 'Find Your Receipt',
        'search_btn' => 'Search',
        'placeholder' => 'Enter Booking ID e.g. YC-123456',
        'not_found' => 'Booking not found. Please verify your ID.',
        'status_pending' => 'PENDING',
        'status_confirmed' => 'PAID',
        'status_cancelled' => 'CANCELLED',
        'download_png' => 'Download PNG',
    ]
];
$text = $t[$lang];

$booking = null;
$error = '';
$search_id = isset($_GET['id']) ? trim($_GET['id']) : '';

$site_settings = [];
// Removed legacy SQLite path
$pdo = null;

if (true) {
    try {
        require_once __DIR__ . '/dashboard/config/db.php';
        $pdo = $conn;
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $stmt_settings = $pdo->query("SELECT setting_key, setting_value FROM settings");
        if ($stmt_settings) {
            $site_settings = $stmt_settings->fetchAll(PDO::FETCH_KEY_PAIR);
        }
    } catch (Exception $e) {}
}

$hotel_name = isset($site_settings['hotel_name']) ? $site_settings['hotel_name'] :  'Yuncha Valley';
$hotel_logo = !empty($site_settings['hotel_logo']) ? 'dashboard/uploads/' . $site_settings['hotel_logo'] : 'img/logo.png';
$hotel_address = isset($site_settings['address']) ? $site_settings['address'] :  '123 Tea Mountain Rd, Chiang Rai, TH';
$hotel_phone = isset($site_settings['phone']) ? $site_settings['phone'] :  '+66 80 342 9396';
$hotel_email = isset($site_settings['email']) ? $site_settings['email'] :  'contact@yunchavalley.com';
$receipt_note = isset($site_settings['receipt_note']) ? $site_settings['receipt_note'] :  "<li><strong>Check-in:</strong> 14:00 onwards. <strong>Check-out:</strong> before 12:00.</li>\n<li>A security deposit of <strong>500 THB</strong> is required upon check-in.</li>\n<li>Cancellation Policy: Free cancellation up to 7 days before check-in.</li>\n<li>This is a computer-generated document. No signature is required.</li>";


if (!empty($search_id) && $pdo) {
    try {
        $stmt = $pdo->prepare("
                SELECT b.*, t.type_name as room_name_th, t.type_name as room_name_en, c.first_name, c.last_name, c.phone as customer_phone, c.email as customer_email, r.room_number, r.base_price, r.high_price, r.holiday_price, r.extra_bed_price, r.discount_percent
                FROM bookings b
                LEFT JOIN rooms r ON b.room_id = r.id
                LEFT JOIN room_types t ON r.room_type_id = t.id
                LEFT JOIN customers c ON b.customer_id = c.id
                WHERE b.booking_ref = ?
            ");
            $stmt->execute([$search_id]);
            $booking = $stmt->fetch(PDO::FETCH_ASSOC);
            
            if (!$booking) {
                $error = $text['not_found'];
            } else {
                $booking['customer_name'] = trim($booking['first_name'] . ' ' . $booking['last_name']);
                
                // Calculate dynamic pricing breakdown
                $weekday_count = 0;
                $weekend_count = 0;
                $holiday_count = 0;

                $inDate = new DateTime($booking['check_in']);
                $outDate = new DateTime($booking['check_out']);
                $nights = $inDate->diff($outDate)->days;
                $currentDate = clone $inDate;
                
                $holiday_dates_arr = array_map('trim', explode(',', isset($site_settings['holiday_dates']) ? $site_settings['holiday_dates'] :  ''));

                $base_price = (float)(isset($booking['base_price']) ? $booking['base_price'] :  0);
                $high_price = (float)(isset($booking['high_price']) ? $booking['high_price'] :  $base_price);
                $holiday_price = (float)(isset($booking['holiday_price']) ? $booking['holiday_price'] :  $base_price);
                
                $extra_beds = (int)(isset($booking['extra_bed']) ? $booking['extra_bed'] :  0);
                $extra_bed_price = (float)(isset($booking['extra_bed_price']) ? $booking['extra_bed_price'] :  0);
                
                $discount_percent = (float)(isset($booking['discount_percent']) ? $booking['discount_percent'] :  0);

                for ($i = 0; $i < $nights; $i++) {
                    $dateStr = $currentDate->format('Y-m-d');
                    $dayOfWeek = $currentDate->format('N');
                    
                    if (in_array($dateStr, $holiday_dates_arr) && $holiday_price > 0) {
                        $holiday_count++;
                    } elseif ($dayOfWeek == 5 || $dayOfWeek == 6) {
                        $weekend_count++;
                    } else {
                        $weekday_count++;
                    }
                    $currentDate->modify('+1 day');
                }
                
                $total_calculated = ($weekday_count * $base_price) + ($weekend_count * $high_price) + ($holiday_count * $holiday_price);
                $discount_amount = $total_calculated * ($discount_percent / 100);
                $extra_bed_total = $nights * $extra_beds * $extra_bed_price;
                $total_expected = $total_calculated - $discount_amount + $extra_bed_total;
                
                $adjustment = (float)$booking['total_price'] - $total_expected;
            }
        } catch (Exception $e) {
            $error = 'Database error.';
        }
    }
?>
<!DOCTYPE html>
<html lang="<?= $lang ?>">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    <title><?= $text['title'] ?> | YUNCHA VALLEY</title>
    
    <!-- Google Fonts -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@400;500;600;700&family=Noto+Serif+SC:wght@400;500;600&family=Noto+Serif+Thai:wght@300;400;500;600&family=Prompt:wght@300;400;500;600&display=swap" rel="stylesheet">
    
    <!-- CDNs -->
    <script src="https://cdn.tailwindcss.com"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js"></script>
    
    <style>
        body {
            background-color: #f3f4f6; /* Light gray background to show A4 edges */
            color: #111827; /* Dark text */
            font-family: 'Prompt', sans-serif;
            overflow-x: hidden;
        }
        .font-cinzel { font-family: 'Cinzel', serif; }
        .font-serif-thai { font-family: 'Noto Serif Thai', serif; }

        .btn-gradient-glow {
            background: linear-gradient(135deg, #d97706, #b45309) !important;
            color: white !important;
            transition: all 0.3s ease !important;
            box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);
        }
        .btn-gradient-glow:hover {
            transform: translateY(-2px) !important;
            box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05);
        }

        /* A4 Document Setup */
        .receipt-page {
            width: 210mm;
            min-height: 297mm;
            margin: 120px auto 40px auto;
            background: white;
            box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04);
            padding: 30px 40px;
            position: relative;
        }

        .status-badge {
            font-size: 14px;
            padding: 6px 16px;
            border-radius: 4px;
            text-transform: uppercase;
            font-weight: bold;
            letter-spacing: 2px;
            font-family: 'Cinzel', serif;
        }
        .status-pending { background-color: #fef3c7; border: 1px solid #f59e0b; color: #b45309; }
        .status-confirmed { background-color: #d1fae5; border: 1px solid #10b981; color: #047857; }
        .status-cancelled { background-color: #fee2e2; border: 1px solid #ef4444; color: #b91c1c; }

        /* Print Settings */
        @page {
            size: A4;
            margin: 0;
        }
        @media print {
            body { background-color: white !important; margin: 0; padding: 0; overflow: hidden; }
            .no-print { display: none !important; }
            .receipt-page { 
                margin: 0 !important;
                padding: 15mm 15mm 0mm 15mm !important; /* ลด padding เพื่อไม่ให้ล้นหน้ากระดาษ */
                box-shadow: none !important; 
                width: 100% !important;
                height: auto !important;
                min-height: auto !important;
                page-break-after: avoid;
                page-break-inside: avoid;
            }
            html { height: 100%; overflow: hidden; }
        }
    </style>
</head>
<body class="antialiased">

    <!-- Toolbar (No Print) -->
    <div class="fixed top-0 left-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-gray-200 p-4 no-print flex justify-between items-center px-4 md:px-8 shadow-sm">
        <a href="<?= isset($_SESSION['customer_email']) ? 'my-bookings.php' : 'index.php' ?>" class="flex items-center gap-2">
            <svg class="w-5 h-5 text-amber-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7"></path></svg>
            <span class="font-cinzel text-sm text-gray-900 font-bold tracking-widest uppercase"><?= htmlspecialchars($hotel_name) ?></span>
        </a>
        <div class="flex gap-2 md:gap-4">
            <button id="download-png-btn" class="px-4 py-2 rounded-lg text-xs font-bold bg-white/20 border border-white/30 hover:bg-white/30 text-white uppercase tracking-widest flex items-center gap-2 transition backdrop-blur-md">
                🖼️ <?= $text['download_png'] ?>
            </button>
        </div>
    </div>

    <div class="flex flex-col items-center justify-center px-4">
        
        <?php if (!$booking): ?>
            <!-- Search Form -->
            <div class="w-full max-w-md bg-white border border-gray-200 rounded-2xl p-8 shadow-lg mt-32 no-print">
                <h1 class="font-cinzel text-2xl font-bold text-center text-amber-600 tracking-widest uppercase mb-6"><?= $text['title'] ?></h1>
                <form action="receipt.php" method="GET" class="flex flex-col gap-4">
                    <input type="text" name="id" value="<?= htmlspecialchars($search_id) ?>" placeholder="<?= $text['placeholder'] ?>" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-4 text-sm text-center text-gray-900 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors uppercase">
                    <button type="submit" class="w-full py-4 rounded-xl font-bold btn-gradient-glow uppercase tracking-[0.2em] text-sm">
                        <?= $text['search_btn'] ?>
                    </button>
                </form>
                <?php if ($error): ?>
                    <p class="text-red-600 text-xs text-center mt-4 bg-red-50 py-3 rounded-lg border border-red-100"><?= $error ?></p>
                <?php endif; ?>
            </div>
        <?php else: 
            $status_map = ['pending' => $text['status_pending'], 'confirmed' => $text['status_confirmed'], 'cancelled' => $text['status_cancelled']];
            $status_class = 'status-' . $booking['status'];
            $room_name = $lang === 'en' ? $booking['room_name_en'] : $booking['room_name_th'];

            $total_paid = $booking['total_price'];
            $subtotal = $total_paid / 1.07;
            $vat = $total_paid - $subtotal;
        ?>

        <!-- Luxury A4 White Receipt -->
        <div id="receipt-document" class="receipt-page">
            
            <!-- 1. Header -->
            <div class="flex justify-between items-start border-b border-gray-100 pb-4 mb-4">
                <div class="flex items-center gap-5">
                    <!-- Logo -->
                    <img src="<?= $hotel_logo ?>" alt="<?= htmlspecialchars($hotel_name) ?> Logo" class="w-16 h-16 object-contain bg-white" onerror="this.src='https://via.placeholder.com/80x80/000000/FFFFFF?text=Logo'">
                    <div>
                        <h2 class="font-cinzel text-3xl font-bold text-gray-900 tracking-widest uppercase leading-none"><?= htmlspecialchars($hotel_name) ?></h2>
                        <p class="font-serif-thai text-sm text-amber-600 mt-1 italic tracking-wider">Luxury Retreat</p>
                        <p class="text-[10px] text-gray-500 mt-2 tracking-widest leading-relaxed">
                            <?= nl2br(htmlspecialchars($hotel_address)) ?><br>
                            T:<br><?= nl2br(htmlspecialchars($hotel_phone)) ?><br>
                            E: <?= htmlspecialchars($hotel_email) ?><br>
                            W: www.yunchavalley.com
                        </p>
                    </div>
                </div>
                <div class="flex flex-col items-end gap-3 mt-6 md:mt-0 text-right">
                    <h1 class="font-cinzel text-4xl font-bold text-gray-200 tracking-widest uppercase">RECEIPT</h1>
                    <span class="status-badge <?= $status_class ?> mt-1"><?= isset($status_map[$booking['status']]) ? $status_map[$booking['status']] :  $booking['status'] ?></span>
                    <span class="text-xs text-gray-600 font-mono mt-2">ID: <span class="text-gray-900 font-bold"><?= isset($booking['booking_ref']) ? $booking['booking_ref'] :  $booking['id'] ?></span></span>
                    <span class="text-[10px] text-gray-500 font-mono">Date: <?= date('d M Y, H:i', strtotime($booking['created_at'])) ?></span>
                </div>
            </div>

            <!-- 2. Guest Info -->
            <div class="grid grid-cols-2 gap-12 mb-10">
                <div>
                    <h3 class="text-xs font-bold text-amber-600 uppercase tracking-widest mb-4 border-b border-gray-200 pb-2 inline-block w-full">Billed To / Guest Info</h3>
                    <div class="flex flex-col gap-2 text-xs font-prompt text-gray-800">
                        <div class="flex justify-between"><span class="text-gray-500">Name:</span> <span class="font-semibold"><?= htmlspecialchars($booking['customer_name']) ?></span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Phone:</span> <span class="font-mono"><?= htmlspecialchars($booking['customer_phone']) ?></span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Email:</span> <span class="font-mono"><?= htmlspecialchars($booking['customer_email']) ?></span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Passport / ID:</span> <span class="font-mono">-</span></div>
                    </div>
                </div>
                <div>
                    <h3 class="text-xs font-bold text-amber-600 uppercase tracking-widest mb-4 border-b border-gray-200 pb-2 inline-block w-full">Reservation Details</h3>
                    <div class="flex flex-col gap-2 text-xs font-prompt text-gray-800">
                        <div class="flex justify-between"><span class="text-gray-500">Check-in:</span> <span class="font-semibold"><?= (new DateTime($booking['check_in']))->format('d M Y') ?> (14:00)</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Check-out:</span> <span class="font-semibold"><?= (new DateTime($booking['check_out']))->format('d M Y') ?> (12:00)</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Room Type:</span> <span class="font-semibold"><?= $room_name ?> (<?= htmlspecialchars(isset($booking['room_number']) ? $booking['room_number'] :  'N/A') ?>)</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Duration:</span> <span class="font-semibold"><?= $nights ?> Nights</span></div>
                    </div>
                </div>
            </div>

            <!-- 3. Itemized Table -->
            <div class="mb-6">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b-2 border-gray-800 text-[10px] text-gray-900 uppercase tracking-widest font-cinzel font-bold bg-gray-50">
                            <th class="py-3 px-4 font-bold rounded-tl-lg">Item Description</th>
                            <th class="py-3 px-4 font-bold text-center">Qty (Nights)</th>
                            <th class="py-3 px-4 font-bold text-right">Unit Price</th>
                            <th class="py-3 px-4 font-bold text-right rounded-tr-lg">Amount (THB)</th>
                        </tr>
                    </thead>
                    <tbody class="text-sm font-prompt text-gray-800">
                        <?php if ($weekday_count > 0): ?>
                        <tr class="border-b border-gray-100">
                            <td class="py-4 px-4">
                                <span class="block font-semibold text-gray-900">Room Rate - Weekday</span>
                                <span class="text-[10px] text-gray-500">Sunday - Thursday</span>
                            </td>
                            <td class="py-4 px-4 text-center"><?= $weekday_count ?></td>
                            <td class="py-4 px-4 text-right font-mono"><?= number_format($base_price, 2) ?></td>
                            <td class="py-4 px-4 text-right font-mono text-gray-900"><?= number_format($weekday_count * $base_price, 2) ?></td>
                        </tr>
                        <?php endif; ?>
                        
                        <?php if ($weekend_count > 0): ?>
                        <tr class="border-b border-gray-100">
                            <td class="py-4 px-4">
                                <span class="block font-semibold text-gray-900">Room Rate - Weekend</span>
                                <span class="text-[10px] text-gray-500">Friday - Saturday</span>
                            </td>
                            <td class="py-4 px-4 text-center"><?= $weekend_count ?></td>
                            <td class="py-4 px-4 text-right font-mono"><?= number_format($high_price, 2) ?></td>
                            <td class="py-4 px-4 text-right font-mono text-gray-900"><?= number_format($weekend_count * $high_price, 2) ?></td>
                        </tr>
                        <?php endif; ?>

                        <?php if ($holiday_count > 0): ?>
                        <tr class="border-b border-gray-100">
                            <td class="py-4 px-4">
                                <span class="block font-semibold text-gray-900">Room Rate - Public Holiday</span>
                                <span class="text-[10px] text-gray-500">Peak Season Surcharge</span>
                            </td>
                            <td class="py-4 px-4 text-center"><?= $holiday_count ?></td>
                            <td class="py-4 px-4 text-right font-mono"><?= number_format($holiday_price, 2) ?></td>
                            <td class="py-4 px-4 text-right font-mono text-gray-900"><?= number_format($holiday_count * $holiday_price, 2) ?></td>
                        </tr>
                        <?php endif; ?>

                        <?php if ($extra_beds > 0): ?>
                        <tr class="border-b border-gray-100">
                            <td class="py-4 px-4">
                                <span class="block font-semibold text-gray-900">Extra Bed(s)</span>
                                <span class="text-[10px] text-gray-500"><?= $extra_beds ?> Bed(s) x <?= $nights ?> Night(s)</span>
                            </td>
                            <td class="py-4 px-4 text-center"><?= $extra_beds * $nights ?></td>
                            <td class="py-4 px-4 text-right font-mono"><?= number_format($extra_bed_price, 2) ?></td>
                            <td class="py-4 px-4 text-right font-mono text-gray-900"><?= number_format($extra_beds * $nights * $extra_bed_price, 2) ?></td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>

            <!-- 4. Summary -->
            <div class="flex flex-col md:flex-row justify-between items-end gap-8 mb-6">
                
                <!-- Payment Info -->
                <div class="w-full md:w-auto">
                    <h4 class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 font-cinzel">Payment Method</h4>
                    <div class="border border-gray-200 p-3 rounded-lg flex items-center gap-3">
                        <div class="text-xl">💳</div>
                        <div class="flex flex-col">
                            <span class="text-xs font-bold text-gray-900">Bank Transfer / PromptPay</span>
                            <span class="text-[10px] text-gray-500">Verified & Approved</span>
                        </div>
                    </div>
                </div>

                <!-- Totals -->
                <div class="w-full md:w-1/2 ml-auto">
                    <div class="flex flex-col gap-3 text-sm font-prompt text-gray-800 bg-gray-50 p-6 rounded-xl border border-gray-100">
                        <div class="flex justify-between">
                            <span class="text-gray-500 uppercase tracking-widest text-xs">Subtotal</span>
                            <span class="font-mono"><?= number_format($subtotal, 2) ?></span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-500 uppercase tracking-widest text-xs">VAT (7%) Included</span>
                            <span class="font-mono"><?= number_format($vat, 2) ?></span>
                        </div>
                        <?php if ($discount_amount > 0): ?>
                        <div class="flex justify-between">
                            <span class="text-gray-500 uppercase tracking-widest text-xs">Discount (<?= $discount_percent ?>%)</span>
                            <span class="font-mono text-red-500">-<?= number_format($discount_amount, 2) ?></span>
                        </div>
                        <?php endif; ?>
                        <?php if (abs($adjustment) > 0.01): ?>
                        <div class="flex justify-between">
                            <span class="text-gray-500 uppercase tracking-widest text-xs">Adjustment</span>
                            <span class="font-mono"><?= $adjustment > 0 ? '+' : '' ?><?= number_format($adjustment, 2) ?></span>
                        </div>
                        <?php endif; ?>
                        <div class="flex justify-between items-center border-t border-gray-300 pt-4 mt-2">
                            <span class="text-gray-900 uppercase tracking-widest font-bold font-cinzel text-lg">Total Amount</span>
                            <span class="font-mono text-3xl font-bold text-amber-600">฿<?= number_format($total_paid, 2) ?></span>
                        </div>
                    </div>
                </div>
            </div>

            <!-- 5. Notes -->
            <div class="mb-6">
                <h4 class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 font-cinzel border-b border-gray-200 pb-2 inline-block">Terms & Conditions</h4>
                <ul class="text-[10px] text-gray-600 list-disc pl-4 space-y-1 font-prompt mt-2">
                    <?= $receipt_note ?>
                </ul>
            </div>

            <!-- 6. Footer (Absolute bottom of A4 if possible, or just normal flow) -->
            <div class="text-center mt-auto border-t border-gray-200 pt-4">
                <p class="font-cinzel text-lg text-amber-600 italic mb-3">"<?= htmlspecialchars(isset($site_settings['msg_receipt_footer']) ? $site_settings['msg_receipt_footer'] :  'Thank you for staying with ' . $hotel_name) ?>"</p>
                <div class="flex justify-center items-center gap-2 opacity-50">
                    <div class="w-12 h-[1px] bg-gray-400"></div>
                    <span class="font-cinzel text-lg text-gray-400">茶</span>
                    <div class="w-12 h-[1px] bg-gray-400"></div>
                </div>
                <p class="text-[9px] text-gray-400 mt-4 tracking-widest uppercase"><?= htmlspecialchars(strtoupper($hotel_name)) ?> CO., LTD. | TAX ID: <?= htmlspecialchars(isset($site_settings['tax_id']) ? $site_settings['tax_id'] :  '0123456789012') ?></p>
            </div>
            
        </div>
        <?php endif; ?>
    </div>

    <script>
        const downloadPngBtn = document.getElementById('download-png-btn');
        
        if (downloadPngBtn) {
            downloadPngBtn.addEventListener('click', () => {
                downloadPngBtn.innerHTML = '⏳ Processing...';
                setTimeout(() => {
                    const originalDoc = document.getElementById('receipt-document');
                    const clone = originalDoc.cloneNode(true);
                    
                    clone.style.position = 'absolute';
                    clone.style.top = '0';
                    clone.style.left = '0';
                    clone.style.zIndex = '-9999';
                    document.body.appendChild(clone);
                    
                    html2canvas(clone, {
                        scale: 2,
                        backgroundColor: '#ffffff',
                        useCORS: true,
                        scrollY: 0,
                        scrollX: 0,
                        windowWidth: 1024,
                        width: 800
                    }).then(canvas => {
                        const link = document.createElement('a');
                        link.download = `Yuncha_Receipt_<?= isset($booking['id']) ? $booking['id'] :  'Receipt' ?>.png`;
                        link.href = canvas.toDataURL('image/png');
                        link.click();
                        document.body.removeChild(clone);
                        downloadPngBtn.innerHTML = '🖼️ <?= $text["download_png"] ?>';
                    }).catch(err => {
                        console.error(err);
                        document.body.removeChild(clone);
                        downloadPngBtn.innerHTML = '🖼️ <?= $text["download_png"] ?>';
                    });
                }, 100);
            });
        }
    </script>
</body>
</html>
